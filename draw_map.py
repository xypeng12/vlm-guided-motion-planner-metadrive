...
parser.add_argument("--map",  default=None,
                    help="Map preset name OR raw block sequence (e.g. SSrSSSRS). "
                         "Omit to draw the custom highway.")
...
if args.map is None:
    ...
    # treat as raw block sequence
    name = args.map
    print(f"Building map from sequence '{name}' (seed={args.seed}) …")
    env = make_env(
        map_preset=args.map,
        num_lanes=args.lanes,
        traffic_density=0.0,
